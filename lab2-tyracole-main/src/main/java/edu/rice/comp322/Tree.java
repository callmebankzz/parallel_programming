...
public interface Tree<T> {
    ...
    class Node<T> implements Tree<T> {
        ...
        public <R> Tree<R> map(Function<T, R> f) {
            ...
            // Must apply function f to all nodes in the tree
            return makeNode(f.apply(value()), children().map(t -> t.map(f)));
        }
        ...
        public Tree<T> filter(Predicate<T> p) {
            ...
            /*
             * This filters out all the children that do not satisfy the predicate, followed by filtering out any
             * children that are empty.
             */
            var newChildren = children().map(t -> t.filter(p)).filter(t -> !t.isEmpty());
            ...
            /*
             * Root value is not valid and all children are empty must be checked because you cannot
             * call makeTreeHelper on empty tree list.
             */
            else if (newChildren.isEmpty()){
                return empty();
            }
            // Root value is not valid, but there are nonempty children, so makeTreeHelper can be called.
            else {
     ...
     */
    static <R> Tree<R> makeTreeHelper(GList<Tree<R>> treeList) {
        if (treeList.isEmpty()) {
            return empty();
        } else {
