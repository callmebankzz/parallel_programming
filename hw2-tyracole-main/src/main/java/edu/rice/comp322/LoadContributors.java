...
public interface LoadContributors {
    ...
    default int loadContributorsPar(String username, String password, String org)
        ...
        //Get the contributors for each repo
        List<HjFuture<List<User>>> users = new ArrayList<>();
            ...
            asyncAwait(users, () -> {
                users.add(future.safeGet());
            });
        //Aggregate the number of contributions for each user and sort the users in descending order of contributions
        System.out.println("Aggregating Results");
        var aggregatedUsers = users.parallelStream().map(u -> u.contributions).sorted(Comparator.reverseOrder());
